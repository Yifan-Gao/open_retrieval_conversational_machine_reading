...
class DiscernForMultipleSequence(RobertaPreTrainedModel):
    ...
    def __init__(self, config, lambda_entailment=3.0, sequence_transformer_layer=3,):
        ...
        self.w_selfattn = nn.Linear(config.hidden_size * 2, 1)
        self.w_output = nn.Linear(config.hidden_size * 2, 3)
    ...
    def forward(
        ...
        # v10 (from v9): discern like ablation: add transformer layers + + rule cls + entailment vector
        user_rule_input, user_rule_mask, rule_logits_mask = [], [], []
        for idx, output in enumerate(outputs[0]):
            user_rule_idx = torch.cat([user_idx[idx], rule_idx[idx]], dim=-1)
            user_rule_input.append(torch.index_select(output, 0, user_rule_idx))
            user_rule_mask.append(torch.tensor([False] * user_rule_idx.shape[0], dtype=torch.uint8, device=self.device))
            rule_logits_mask.append(torch.tensor([1] * rule_idx[idx].shape[0], dtype=torch.bool, device=self.device))

        rule_logits_mask = nn.utils.rnn.pad_sequence(rule_logits_mask, batch_first=True).unsqueeze(-1)  # [bz * seqlen * 1]
        ...
        entail_logits = self.w_entail(self.dropout(rule_out))  # [bz * seqlen * 3]
        entail_state = torch.matmul(entail_logits, self.entail_emb)
        cat_state = torch.cat([entail_state, rule_out], dim=-1)
        logits = self.logic_op(cat_state, rule_logits_mask)  # [bz, 3]
