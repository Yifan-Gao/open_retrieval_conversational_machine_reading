...
def main():
    ...
    def _preprocess_function_snippet(result, top_k_snippet_ids, example, top_k_snippet_scores):
        ...
        if snippet2id[example['snippet']] in top_k_snippet_ids:
            result['top_k_recall'] = 1
        else:
            result['top_k_recall'] = 0
    ...
    logger.info('Train: recall {:.2f}'.format(
        np.mean(dataset_train['top_k_recall'])*100,
    ))
    ...
    logger.info('Validation: recall {:.2f}'.format(
        np.mean(dataset_validation['top_k_recall'])*100,
    ))
    logger.info('Test: recall {:.2f}'.format(
        np.mean(dataset_test['top_k_recall'])*100,
    ))
