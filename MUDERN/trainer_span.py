...
class DiscernDataCollatorWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        batch_keys = ["input_ids", "attention_mask", "start_position", "end_position",]
        ...
        for _k in remaining_keys:
            batch[_k] = torch.nn.utils.rnn.pad_sequence([torch.tensor(fea[_k], dtype=torch.long) for fea in features], batch_first=True)
...
@dataclass
class DiscernDataCollatorWithPaddingEvaluation:
    """
    Data collator that will dynamically pad the inputs received.
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        for _k in remaining_keys:
            batch[_k] = torch.nn.utils.rnn.pad_sequence([torch.tensor(fea[_k], dtype=torch.long) for fea in features], batch_first=True)
        batch["start_position"] = None
        batch["end_position"] = None
        # batch["label_entail"] = nn.utils.rnn.pad_sequence(batch["label_entail"], batch_first=True, padding_value=-100)
        return batch


class DiscernSpanTrainer(Trainer):
    ...
    def evaluate(self, eval_dataset=None, eval_examples=None, ignore_keys=None):
        ...
        ignore_keys = ['loss'] if ignore_keys is None else ignore_keys
    ...
    def predict(self, eval_dataset=None, eval_examples=None, ignore_keys=None):
        ...
        if self.post_process_function is not None and self.compute_metrics is not None:
            eval_preds = self.post_process_function(eval_examples, eval_dataset, output.predictions)
            metrics = self.compute_metrics(eval_preds, eval_dataset)
            self.log(metrics)
        else:
            metrics = {}

        return (eval_preds, metrics)


def postprocess_span_predictions(
    examples,
    features,
    ...
    """
    def get_top_k(probs, k, sent_start, sent_end):
        p = list(enumerate(probs.tolist()))[sent_start : sent_end]
        p.sort(key=lambda tup: tup[1], reverse=True)
        return p[:k]

    def rule_softmax(logit, start, end):
        logit[:start] = -100000.0
        logit = logit[:end]
    ...
    for idx, (id, pstart_i, pend_i, input_ids_i, attention_mask_i, rule_idx_i) in enumerate(
            zip(examples['utterance_id'], all_start_logits, all_end_logits,
                examples['input_ids'], examples['attention_mask'],
                examples['rule_idx'],)):
        pred_i = {'id': id}

        current_snippet_start, current_snippet_end = rule_idx_i[0], len(attention_mask_i)
        pstart_i = rule_softmax(pstart_i, current_snippet_start, current_snippet_end)
        pend_i = rule_softmax(pend_i, current_snippet_start, current_snippet_end)

        # start inclusive, end exclusive
        sentence_start = np.array(rule_idx_i) + 1  # +1 to skip the cls token, inclusive
        sentence_end = np.array(rule_idx_i[1:] + [current_snippet_end])  # exclusive
        top_preds = []
        top_k = n_best_size
        for jdx, (sent_s, sent_e) in enumerate(zip(sentence_start, sentence_end)):
            top_start = get_top_k(pstart_i, top_k, sent_s, sent_e)
            top_end = get_top_k(pend_i, top_k, sent_s, sent_e)
            for s, ps in top_start:
                for e, pe in top_end:
                    if s <= e <= s + max_answer_length:
                        top_preds.append((jdx, s, e, ps * pe))
        top_preds = sorted(top_preds, key=lambda tup: tup[-1], reverse=True)[:top_k]
        top_answers = [(tokenizer.decode(input_ids_i[s:e + 1]), j, s, e, p) for j, s, e, p in top_preds]
        top_ans, top_j, top_s, top_e, top_p = top_answers[0]
        pred_i['answer_span_start'] = top_s
        pred_i['answer_span_end'] = top_e
        pred_i['answer'] = top_ans.strip()
