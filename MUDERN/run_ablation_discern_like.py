...
from model_ablation_discern_like import DiscernForMultipleSequence
from trainer import DiscernDataCollatorWithPadding, DiscernTrainer
...
def main():
    ...
    model = DiscernForMultipleSequence.from_pretrained(
        ...
        revision=model_args.model_revision,
        use_auth_token=True if model_args.use_auth_token else None,
        sequence_transformer_layer=model_args.sequence_transformer_layer,
    ...
    def _preprocess_function_user_info(example):
        ...
        result = {
            ...
            'gold_rule_idx_mask': [],  # gold rule relevant index
            'input_ids': [],
            'label_entail': [],
        }
    ...
    def _preprocess_function_snippet(result, top_k_snippet_ids, example, top_k_snippet_scores):
        ...
        for snippet_id, snippet_score in zip(top_k_snippet_ids, top_k_snippet_scores):
            ...
            for clause_id, edus in enumerate(m['edu_t']):
                for edu_id, edu in enumerate(edus):
                    ...
                    result['gold_rule_idx_mask'].append(1) if is_gold else result['gold_rule_idx_mask'].append(0)
                    result['input_ids'] += [cls_token_id] + edu
                    result['retrieval_score'].append(snippet_score)
            ...
            assert len(result['label_entail']) == len(result['rule_idx']) == len(result['gold_rule_idx_mask']) == len(result['retrieval_score'])
        ...
        result['gold_rule_idx_mask'] = result['gold_rule_idx_mask'][:num_rule_cls]
        result['retrieval_score'] = result['retrieval_score'][:num_rule_cls]
    ...
    # Train: avg 6.24 min 13 max 1 snippets
    ...
    # Validation: avg 6.31 / max 12 / min 2 snippets
    ...
    # Test: avg 6.15 / max 12 / min 1 snippets
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        metrics["micro_accuracy"] = float("{:.2f}".format(micro_accuracy * 100))  # int(100 * micro_accuracy) / 100
        ...
        metrics["macro_accuracy"] = float("{:.2f}".format(macro_accuracy * 100))  # int(100 * macro_accuracy) / 100
        metrics["combined"] = float("{:.2f}".format(macro_accuracy * micro_accuracy * 100))
        metrics["confmat"] = conf_mat.tolist()
        metrics["classwise_accuracy_more"] = float("{:.2f}".format(class_wise_accuracy[0] * 100))
        metrics["classwise_accuracy_no"] = float("{:.2f}".format(class_wise_accuracy[1] * 100))
        metrics["classwise_accuracy_yes"] = float("{:.2f}".format(class_wise_accuracy[2] * 100))

        # entailment tracking
        # if hasattr(p, "predictions_entail") and hasattr(p, "label_ids_entail"):
        #     entail_preds_padded = np.argmax(p.predictions_entail.reshape(-1, 3), axis=1).tolist()
        #     entail_golds_padded = p.label_ids_entail.reshape(-1).tolist()
        #     entail_preds, entail_golds = [], []
        #     for epp, egp in zip(entail_preds_padded, entail_golds_padded):
        #         if egp != -100:
        #             entail_preds.append(epp)
        #             entail_golds.append(egp)
        #
