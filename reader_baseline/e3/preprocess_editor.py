...
def create_split(trees, vocab, max_len=300, train=True):
    ...
    for k in tqdm(keys):
        ...
        snippet = v['t_snippet']
        curr_q_str = []
        for q_str in v['questions']:
            if q_str not in q_str_seen:
                curr_q_str.append(q_str)
                q_str_seen.add(q_str)
        for q_str in curr_q_str:
            q_tok = v['questions'][q_str]
...
if __name__ == '__main__':
    ...
    with open('sharc/trees_test.json') as f:
        test_trees = json.load(f)
    train_proc = torch.load('sharc/proc_train.pt')
    dev_proc = torch.load('sharc/proc_dev.pt')
    test_proc = torch.load('sharc/proc_test.pt')
    ...
    torch.save(test, dout + '/proc_test.pt')
