...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_split', default='dev', help='split for inference')
    ...
    if args.test:
        test_data = torch.load('{}/proc_{}.pt'.format(args.data, args.test_split))
        preds = model.run_pred(test_data)
        metrics = model.compute_metrics(preds, test_data)
        pprint(metrics)
    else:
