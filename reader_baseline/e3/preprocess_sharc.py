...
def compute_metrics(preds, data):
    ...
    with NamedTemporaryFile('w') as fp, NamedTemporaryFile('w') as fg:
        ...
        results['combined'] = results['macro_accuracy'] * results['fscore_bleu_4']
        return results
...
if __name__ == '__main__':
    ...
    for split in ['test', 'dev', 'train', 'dev_seen', 'dev_unseen', 'test_seen', 'test_unseen']:
        ...
        with open('sharc/json/{}.json'.format(fsplit)) as f:
            ...
            if not os.path.isfile(fproc) or FORCE:
                ...
                assert ex['answer'].lower() != 'irrelevant'
                for ex in data:
