...
if __name__ == '__main__':
    ...
    if args.test:
        preds = model.run_pred(data['test'])
        print('saving {}'.format(os.path.join(args.dsave, 'test.preds.json')))
        with open(os.path.join(args.dsave, 'test.preds.json'), 'wt') as f:
            json.dump(preds, f, indent=2)
        metrics = model.compute_metrics(preds, data['test'])
        pprint(metrics)
    else:
