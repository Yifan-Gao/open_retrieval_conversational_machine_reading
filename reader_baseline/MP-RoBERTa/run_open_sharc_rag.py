...
from trainer import DiscernTrainer
...
def main():
    ...
    def _preprocess_function_snippet(result, top_k_snippet_ids):
        for k in ['input_ids', 'attention_mask']:
            ...
            for snippet_id in top_k_snippet_ids:
                result[k].extend(tokenized_sharc_data['snippets']['<sn> ' + id2snippet[snippet_id]][k] + [ssep_token_id])
    ...
    if training_args.do_eval:
        ...
        for eval_dataset, task in zip(eval_datasets, tasks):
            ...
            output_eval_file = os.path.join(training_args.output_dir, f"{task}_predictions.npy")
            np.save(output_eval_file, output.predictions)
