...
from model import RobertaForMultipleSequenceClassification
...
@dataclass
class DataCollatorForMultipleSnippets:
    """
    ...
    def __call__(self, features):
        ...
        num_snippets = len(features[0]["input_ids"])
        flattened_features = [
            [{k: v[i] for k, v in feature.items()} for i in range(num_snippets)] for feature in features
        ]
        ...
        # Un-flatten
        batch = {k: v.view(batch_size, num_snippets, -1) for k, v in batch.items()}
        # Add back labels
...
def main():
    ...
    config.top_k_snippets = data_args.top_k_snippets
    ...
    model = RobertaForMultipleSequenceClassification.from_pretrained(
    ...
    if os.path.exists(tokenized_sharc_path):
        ...
        questions = list(set(["<qu> " + _ex for dataset_split in datasets.values() for _ex in dataset_split['question']]))
        scenarios = list(set(["<sc> " + _ex for dataset_split in datasets.values() for _ex in dataset_split['scenario']]))
        snippets = list(set(["<sn> " + _ex for dataset_split in datasets.values() for _ex in dataset_split['snippet']]))
        follow_up_questions = list(set(
            ["<fuq> " + fuqa['follow_up_question'] for dataset_split in datasets.values() for _ex in dataset_split['history'] for fuqa in _ex] +
            ["<fuq> " + fuqa['follow_up_question'] for dataset_split in datasets.values() for _ex in dataset_split['evidence'] for fuqa in _ex]))
    ...
    def _preprocess_function_user_info(example):
        ...
        for k in ['input_ids', 'attention_mask']:
            ...
            result[k] = [cls_token_id] + \
                        tokenized_sharc_data['questions']['<qu> ' + example['question']][k] + [ssep_token_id] + \
                        tokenized_sharc_data['scenarios']['<sc> ' + example['scenario']][k] + [ssep_token_id]
            result[k].append(fqa_token_id)
            for _fqa in example['history']:
                result[k].extend(tokenized_sharc_data['follow_up_questions']['<fuq> ' + _fqa['follow_up_question']][k])
                if 'yes' in _fqa['follow_up_answer'].lower():
                    result[k].extend(tokenized_sharc_data['follow_up_answers']["<fua> yes"][k])
                else:
                    result[k].extend(tokenized_sharc_data['follow_up_answers']["<fua> no"][k])
            result[k].append(ssep_token_id)
        return result

    def _preprocess_function_snippet(result, top_k_snippet_ids):
        while len(top_k_snippet_ids) < data_args.top_k_snippets:
            top_k_snippet_ids.insert(0, top_k_snippet_ids[0])
        new_result = {'label': result['label'], 'input_ids': [], 'attention_mask': [],}
        for snippet_id in top_k_snippet_ids:
            for k in ['input_ids', 'attention_mask']:
                ssep_token_id = tokenizer.all_special_ids[tokenizer.all_special_tokens.index('<ssep>')] if k == 'input_ids' else 1
                eos_token_id = tokenizer.eos_token_id if k == 'input_ids' else 1
                new_ids = result[k] + tokenized_sharc_data['snippets']['<sn> ' + id2snippet[snippet_id]][k] + [ssep_token_id]
                # replace the last ssep with eos
                new_ids[-1] = eos_token_id
                # cropping to max_length
                new_ids = new_ids[:data_args.max_seq_length - 2]
                # ensure that all examples have <eos> in the end (required by bart classification)
                if k == 'input_ids' and len(new_ids) == data_args.max_seq_length - 2:
                    new_ids[-1] = eos_token_id
                new_result[k].append(new_ids)
        assert len(new_result['input_ids']) == data_args.top_k_snippets
        return new_result
    ...
    trainer = Trainer(
        ...
        data_collator=DataCollatorForMultipleSnippets(tokenizer=tokenizer),
    )
